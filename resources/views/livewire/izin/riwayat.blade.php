<?php

use Livewire\Volt\Component;
use App\Models\PengajuanIzin;
use Livewire\WithPagination;
use Livewire\Attributes\On; 

new class extends Component {
    use WithPagination;
    public string $nik = '';

    public function mount($nik)
    {
        // dd($nik);
        $this->nik = $nik;
    }

    #[On('refresh')]
    public function getRiwayatIzin()
    {
        return PengajuanIzin::where('nik', $this->nik)->orderBy('tanggal', 'desc')->paginate(10);
    }

    public function headers()
    {
        return [
            ['key' => 'no_pengajuan', 'label' => 'No'],
            ['key' => 'tanggal', 'label' => 'Tanggal'],
            ['key' => 'tanggal_awal', 'label' => 'Tanggal Awal'],
            ['key' => 'tanggal_akhir', 'label' => 'Tanggal Akhir'],
            ['key' => 'jumlah', 'label' => 'Jumlah'],
            ['key' => 'urgensi', 'label' => 'Urgensi'],
            ['key' => 'kepentingan', 'label' => 'Kepentingan'],
            ['key' => 'status', 'label' => 'Status']
        ];
    }

    public function with()
    {
        return [
            'riwayatIzin' => $this->getRiwayatIzin(),
            'headers' => $this->headers()
        ];
    }

}; ?>

<div>
    <x-table :headers="$headers" :rows="$riwayatIzin" striped with-pagination />
</div>
